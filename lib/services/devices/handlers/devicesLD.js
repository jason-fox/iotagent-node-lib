...
const DeviceV2 = require('./devicesV2');
const context = {
...
function updateEntityNgsiLD(deviceData, updatedDevice, callback) {
    ...
    jsonConcat(options.json, DeviceV2.formatAttributes(deviceData.active, false));
    jsonConcat(options.json, DeviceV2.formatAttributes(deviceData.staticAttributes, true));
    jsonConcat(options.json, DeviceV2.formatCommands(deviceData.commands));
...
class DeviceHandlerLD {
    ...
    createInitialEntity(deviceData, newDevice, callback) {
        ...
        jsonConcat(json, DeviceV2.formatAttributes(deviceData.active, false));
        jsonConcat(json, DeviceV2.formatAttributes(deviceData.staticAttributes, true));
        jsonConcat(json, DeviceV2.formatCommands(deviceData.commands));
        ...
        utils.executeWithSecurity(
            options,
            newDevice,
            createInitialEntityHandlerNgsiLD(deviceData, newDevice, callback)
        );
    }

    /**
     * Updates the register of an existing device identified by the Id and Type in the Context Broker, and the internal
     * registry. It uses NGSI-LD.
     *
...
module.exports = DeviceHandlerLD;
