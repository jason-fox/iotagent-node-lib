...
const DeviceHandlerLD = require('./devicesLD');
const DeviceHandlerV2 = require('./devicesV2');

class DeviceHandlerMixed {
    constructor() {
        this.deviceHandlerLD = new DeviceHandlerLD();
        this.deviceHandlerV2 = new DeviceHandlerV2();
    }
    ...
    createInitialEntity(deviceData, newDevice, callback) {
        if (config.checkNgsiLD(deviceData)) {
            this.deviceHandlerLD.createInitialEntity(deviceData, newDevice, callback);
        } else {
            this.deviceHandlerV2.createInitialEntity(deviceData, newDevice, callback);
        }
    }
    ...
    updateRegisterDevice(deviceObj, callback) {
        if (config.checkNgsiLD(deviceObj)) {
            this.deviceHandlerLD.updateRegisterDevice(deviceObj, callback);
        } else {
            this.deviceHandlerV2.updateRegisterDevice(deviceObj, callback);
        }
    }
}

module.exports = DeviceHandlerMixed;
