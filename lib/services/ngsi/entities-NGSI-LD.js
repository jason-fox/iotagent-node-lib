...
function sendQueryValueNgsiLD(entityName, attributes, typeInformation, token, callback) {
    ...
    .catch (error => {
        handler(error, error, error.body);
    });
}
        ...
        function(error, result) {
            if (error) {
                ...
                .catch (error => {
                    ...
                    callback(new errors.EntityGenericError(entityName, typeInformation.type, JSON.parse(errResponse.body), errResponse.statusCode));
                });
            }
        }
    );
