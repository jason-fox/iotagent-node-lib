                    ...
                    if (! error.response ) {
                        ...
                        if (errResponse && (errResponse.statusCode === 403 || errResponse.statusCode === 401)) {
                            ...
                            logger.debug(context, 
                                'Received the following errResponse from the CB:\n\n%s\n\n', errResponse.body);
                            logger.error(context, 
                                'Operation update error connecting to the Context Broker: %j', body);
                            ...
                            if (
                                ...
                                callback(new errors.EntityGenericError(
                                    entityName, typeInformation.type, body, errResponse.statusCode));
                            }
                        } else {
                            callback(new errors.EntityGenericError(
                                entityName, typeInformation.type, body, errResponse.statusCode));
