...
function sendQueryValueNgsiLD(entityName, attributes, typeInformation, token, callback) {
    ...
    if (attributes && attributes.length > 0) {
        options.searchParams = 'attrs=' + attributes.join(',');
    }
    ...
    const handler = generateNGSILDOperationHandler('query', entityName, 
        typeInformation, token, options, function(error, result) {
            if (error) {
                callback(error);
            } else {
                NGSIUtils.applyMiddlewares(NGSIUtils.queryMiddleware, JSON.parse(result), 
                    typeInformation, callback);
            }
        });
    ...
    .catch (error => {
        ...
        const errResponse = error.response || {statusCode: 400, body: {}};
        let body = errResponse.body;
        try {
        ...
        console.error(error);
        callback(new errors.EntityGenericError(entityName, 
            typeInformation.type, body, errResponse.statusCode));
    });
}
    ...
    async.waterfall(
        ...
        function(error, result) {
            if (error) {
                ...
                const handler =
                    generateNGSILDOperationHandler('update', entityName, typeInformation,
                ...
                .catch (error => {
                    ...
                    if (errResponse && (errResponse.statusCode === 403 || errResponse.statusCode === 401)) {
                        logger.debug(context, 'Access forbidden executing update operation');
                        callback(
                    ...
                    } else if (errResponse && body && errResponse.statusCode === 404) {
                        logger.debug(context, 'Received the following errResponse from the CB:\n\n%s\n\n', errResponse.body);

                        logger.error(context, 'Operation update error connecting to the Context Broker: %j', body);
                        ...
                        if (
                            ...
                            callback(new errors.EntityGenericError(entityName, typeInformation.type, body, errResponse.statusCode));
                        }
                    } else {
                        callback(new errors.EntityGenericError(entityName, typeInformation.type, body, errResponse.statusCode));
                    } 
